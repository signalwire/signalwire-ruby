...
- **Relay event field accessors** (under `signalwire.relay.event.*Event.*`):
  Ruby exposes each Python `@dataclass` field as an explicit attr_reader. The
  oracle now records these fields as surface/signature members, so every
  accessor FOLDS by name (the enumerator emits it under the reference field
  name) — they are no longer additions. `DialEvent#call_data` folds via a
  class-scoped rename to the reference `call` field
  (`SURFACE_METHOD_ALIASES`/`SIG_METHOD_ALIASES`). Only each event class's
  synthesized `#__init__` remains a port-only addition (the reference dataclass's
  generated `__init__` is not a griffe-recorded surface member).
...
<!-- agentbase-family fold keys (SURFACE diff folds AgentBase + mixin methods to
this token; the raw signalwire.core.agent_base.AgentBase.* twins above remain for the
unfolded SIGNATURE diff). -->
agentbase-family.agent_id: port-only: Ruby attr_reader for the auto-generated/explicit agent UUID (Python keeps it as `self.agent_id` instance attribute)
agentbase-family.app: ruby-idiom bare-noun reader (D9) over get_app; native `agent.app`, get_ name retained for parity
agentbase-family.as_rack_app: port-only: mixin method collapsed onto SignalWire::AgentBase (Ruby single-inheritance + modules model replaces Python multiple inheritance)
agentbase-family.basic_auth_credentials: ruby-idiom bare-noun reader (D9) over get_basic_auth_credentials; native `agent.basic_auth_credentials`, get_ name retained for parity
agentbase-family.build_main_section: port-only: Ruby SWML "main"-section assembly helper called by #_render_swml_internal (Python assembles the sections inline in its render path; Ruby factors the ordered pre-answer/answer/record/post/ai/post-ai phase build into a named method)
agentbase-family.create_tool_token: port-only: Ruby instance helper that delegates to SessionManager#create_token (Python keeps token creation only in SessionManager)
agentbase-family.default_webhook_url: port-only: Ruby attr_reader for the constructor `default_webhook_url:` arg (Python: `self._default_webhook_url`)
...
agentbase-family.logger: port-only: Ruby attr_reader for the registry's namespaced logger (Python parity: `self.logger`)
agentbase-family.native_functions: port-only: Ruby attr_reader for the constructor `native_functions:` arg (Python: `self.native_functions` list)
agentbase-family.post_prompt: ruby-idiom alias (reader+writer) over get_post_prompt/set_post_prompt; native `agent.post_prompt` / `agent.post_prompt=`
agentbase-family.prompt: ruby-idiom alias (reader) over get_prompt; native `agent.prompt` accessor, Python name retained for parity
agentbase-family.prompt_text: ruby-idiom alias (reader+writer) over get_raw_prompt/set_prompt_text; native `agent.prompt_text` / `agent.prompt_text=`
agentbase-family.rack_app: port-only: Ruby's SWML::Service hosts the tool/web mixin surface + attr_readers + Rack helpers directly on the base service (Python splits these across ToolMixin/WebMixin/AgentBase; folded onto the base is the Ruby idiom)
agentbase-family.render_swml: port-only: mixin method collapsed onto SignalWire::AgentBase (Ruby single-inheritance + modules model replaces Python multiple inheritance)
agentbase-family.signing_key: ruby_idiom_port_only: AgentBase exposes a `signing_key` accessor; Python keeps it as private state (no public getter). Public access in Ruby for testability and middleware mounting.
agentbase-family.skill?: ruby-idiom predicate (D9) over has_skill?; native `agent.skill?(name)`, has_ name retained for parity
agentbase-family.use_pom: port-only: Ruby attr_reader for the constructor `use_pom:` flag (Python: `self._use_pom` private attribute)
