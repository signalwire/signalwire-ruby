...
# Generated full-mock wire tests for the 'messages' REST namespace.
class MessagesGeneratedTest < Minitest::Test
  ...
  def test_messages_create_success
    @client.messages.create(to: 'x', from: 'x')
    last = @mock.last

    assert_equal 'POST', last.method
    assert_equal 'messages.create_message', last.matched_route
  end

  def test_messages_create_error
    @mock.push_scenario('messages.create_message', status: 500, response: { 'error' => 'x' })
    err = assert_raises(SignalWire::REST::SignalWireRestError) { @client.messages.create(to: 'x', from: 'x') }

    assert_equal 500, err.status_code
    assert_equal 500, @mock.last.response_status
    assert_equal 'messages.create_message', @mock.last.matched_route
  end
  ...
  def test_messages_update_success
    ...
    assert_equal 'messages.update_message', last.matched_route
  end
  ...
  def test_messages_update_error
    ...
    assert_equal 500, err.status_code
    assert_equal 500, @mock.last.response_status
    assert_equal 'messages.update_message', @mock.last.matched_route
  end
end
