...
# Generated full-mock wire tests for the 'chat' REST namespace.
class ChatGeneratedTest < Minitest::Test
  ...
  def test_chat_create_token_success
    @client.chat.create_token(ttl: 'x', channels: 'x')
    last = @mock.last

    assert_equal 'POST', last.method
    assert_equal 'chat.create_chat_token', last.matched_route
  end

  def test_chat_create_token_error
    @mock.push_scenario('chat.create_chat_token', status: 500, response: { 'error' => 'x' })
    err = assert_raises(SignalWire::REST::SignalWireRestError) { @client.chat.create_token(ttl: 'x', channels: 'x') }

    assert_equal 500, err.status_code
    assert_equal 500, @mock.last.response_status
    assert_equal 'chat.create_chat_token', @mock.last.matched_route
  end
end
