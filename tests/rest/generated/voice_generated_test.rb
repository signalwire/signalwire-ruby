...
# Generated full-mock wire tests for the 'voice' REST namespace.
class VoiceGeneratedTest < Minitest::Test
  ...
  def test_voice_get_success
    ...
    assert_equal 'voice.get_voice_log', last.matched_route
  end

  def test_voice_get_error
    @mock.push_scenario('voice.get_voice_log', status: 500, response: { 'error' => 'x' })
    err = assert_raises(SignalWire::REST::SignalWireRestError) { @client.logs.voice.get('x') }

    assert_equal 500, err.status_code
    assert_equal 500, @mock.last.response_status
    assert_equal 'voice.get_voice_log', @mock.last.matched_route
  end
  ...
  def test_voice_list_error
    @mock.push_scenario('voice.list_voice_logs', status: 500, response: { 'error' => 'x' })
    err = assert_raises(SignalWire::REST::SignalWireRestError) { @client.logs.voice.list }

    assert_equal 500, err.status_code
    assert_equal 500, @mock.last.response_status
    assert_equal 'voice.list_voice_logs', @mock.last.matched_route
  end
  ...
  def test_voice_list_events_success
    ...
    assert_equal 'GET', last.method
    assert_equal 'voice.list_voice_log_events', last.matched_route
  end

  def test_voice_list_events_error
    @mock.push_scenario('voice.list_voice_log_events', status: 500, response: { 'error' => 'x' })
    err = assert_raises(SignalWire::REST::SignalWireRestError) { @client.logs.voice.list_events('x') }

    assert_equal 500, err.status_code
    assert_equal 500, @mock.last.response_status
    assert_equal 'voice.list_voice_log_events', @mock.last.matched_route
  end
end
