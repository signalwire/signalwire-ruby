...
class SessionManagerTest < Minitest::Test
  ...
  def test_token_is_url_safe_base64
    ...
    refute_match(%r{[+/]}, token, 'Token should be URL-safe Base64')
  end
...
class SessionManagerTamperTest < Minitest::Test
  ...
  def test_tampered_expiry_fails
    ...
    parts[2] = (Time.now.to_i + 999_999).to_s
    tampered_token = Base64.urlsafe_encode64(parts.join('.'), padding: false)
...
class SessionManagerSessionTest < Minitest::Test
  ...
  def test_create_session_generates_when_absent
    ...
    refute_match(%r{[+/=]}, id, 'generated session id must be URL-safe with no padding')
  end
