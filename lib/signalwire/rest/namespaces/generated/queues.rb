# frozen_string_literal: true

# Spec-derived generated surface: wire keys, folded schema constants, and per-schema
# CRUD/data-class size are preserved verbatim; these cops are pruned per file by the
# generator's rubocop pass to exactly those that fire.
# rubocop:disable Layout/LineLength

# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from the SignalWire REST API specifications — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST resource for the 'relay-rest' namespace.

module SignalWire
  module REST
    module Namespaces
      module Generated
        # Queues — REST resource for the relay-rest API (base CrudResource).
        class Queues < SignalWire::REST::CrudResource
          def initialize(http)
            super(http, '/api/relay/rest/queues')
          end

          def create(name: nil, max_size: nil, extras: {}, request_options: nil, **kwargs)
            body = {}
            body['name'] = name unless name.nil?
            body['max_size'] = max_size unless max_size.nil?
            body = body.merge(extras).merge(kwargs)
            @http.post(@base_path, body, request_options: request_options)
          end

          def update(resource_id, name: nil, max_size: nil, extras: {}, request_options: nil, **kwargs)
            body = {}
            body['name'] = name unless name.nil?
            body['max_size'] = max_size unless max_size.nil?
            body = body.merge(extras).merge(kwargs)
            @http.put(_path(resource_id), body, request_options: request_options)
          end

          def delete(resource_id, request_options: nil)
            @http.delete(_path(resource_id), request_options: request_options)
          end

          def list_members(queue_id, request_options: nil, **params)
            @http.get(_path(queue_id, 'members'), params.empty? ? nil : params, request_options: request_options)
          end

          def get_next_member(queue_id, request_options: nil, **params)
            @http.get(_path(queue_id, 'members', 'next'), params.empty? ? nil : params, request_options: request_options)
          end

          def get_member(queue_id, id, request_options: nil, **params)
            @http.get(_path(queue_id, 'members', id), params.empty? ? nil : params, request_options: request_options)
          end
        end
      end
    end
  end
end
# rubocop:enable Layout/LineLength
