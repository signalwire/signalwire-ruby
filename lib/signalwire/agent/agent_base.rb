  ...
  class AgentBase < SWML::Service
    ...
    def on_function_call(name, args, raw_data)
      ...
      if tool[:secure]
        ...
        if call_id && token && !@session_manager.validate_token(name, token, call_id)
    ...
    def _build_mcp_tool_list
      ...
      @tools.each do |name, tool|
        ...
        t['inputSchema'] = if params && !params.empty?
    ...
    def _handle_mcp_request(body)
      ...
      return _mcp_error(req_id, -32_600, 'Invalid JSON-RPC version') unless jsonrpc == '2.0'

      # Each `when` is a distinct JSON-RPC method; two arms may share a body
      # (e.g. a no-op ack) but stay separate to document the protocol surface.
      case method
      when 'initialize'
        {
    ...
    def _build_ai_config
      ...
      if @post_prompt_text && !@post_prompt_text.empty?
        ...
        # post_prompt_url
        ai['post_prompt_url'] = (@post_prompt_url_override || _build_webhook_url('post_prompt'))
      end
      ...
      # default webhook url
      default_url = @web_hook_url_override || _build_webhook_url('swaig',
                                                                 @swaig_query_params.empty? ? nil : @swaig_query_params)
      ...
      Rack::Builder.new do
        ...
        map main_route do
          ...
          run lambda { |env|
            ...
            next agent.send(:_handle_swaig_endpoint, request, request_data, env) if sub_path == '/swaig'
    ...
    def _handle_mcp_endpoint(request_data, _env)
      ...
      unless request_data
        body = JSON.generate(_mcp_error(nil, -32_700, 'Parse error'))
