...
Two known spec gaps mirror the Python reference's bring-up findings (same underlying
spec, same SDK design) but are NOT allowlisted here — a name-only token like
`unknown_query_param:page_size` would over-broadly mask any future real violation on
the many endpoints that DO declare `page_size`:
  ...
  * `page_size` on `relay-rest.list_recordings` — the spec's `list_recordings` op has
    ...
    that the server accepts it. Tracked separately; do NOT strip the test. See
    porting-sdk `fix/recordings-pagination-spec`.
  * `cursor` on `fabric.list_fabric_addresses` — same class: the fabric list ops have
    ...
    generic paginator replays as a `?cursor=` param. Same owner+prime-rails
    adjudication as recordings. Known test-fixture issue tracked separately.
